...
category_one = session.query(Category).filter_by(
    title=category_one.title).one()

category_two = Category(title="Web", user_id=user_one.id)
session.add(category_two)
session.commit()

category_two = session.query(Category).filter_by(
    title=category_two.title).one()

category_three = Category(title="AI", user_id=user_two.id)
session.add(category_three)
session.commit()

category_three = session.query(Category).filter_by(
    title=category_three.title).one()

category_four = Category(title="VR", user_id=user_two.id)
session.add(category_four)
session.commit()

category_four = session.query(Category).filter_by(
    title=category_four.title).one()
...
for i in range(0, 30):
    if i < 10:
        item_one = Item(
            title="%s company %s" %
            (category_one.title, str(
                i %
                4)), description="This is a %s %s company." %
            (str(
                i %
                4), category_one.title), category_id=category_one.id, user_id=user_one.id)
    elif i < 20:
        item_one = Item(
            title="%s company %s" %
            (category_two.title, str(
                i %
                4)), description="This is a %s %s company." %
            (str(
                i %
                4), category_two.title), category_id=category_two.id, user_id=user_one.id)
    elif i < 30:
        item_one = Item(
            title="%s company %s" %
            (category_three.title, str(
                i %
                4)), description="This is a %s %s company." %
            (str(
                i %
                4), category_three.title), category_id=category_three.id, user_id=user_two.id)
    else:
        item_one = Item(
            title="%s company %s" %
            (category_four.title, str(
                i %
                4)), description="This is a %s %s company." %
            (str(
                i %
                4), category_four.title), category_id=category_four.id, user_id=user_two.id)
